...
vnstat_current() {
    vnstat_version="$(dpkg-query -s vnstat | grep "Version: " | cut -d " " -f2)"
...
vnstat_install() {
    sudo wget -q -O "/tmp/vnstat_2.6-3_armhf.deb" "http://raspbian.raspberrypi.org/raspbian/pool/main/v/vnstat/vnstat_2.6-3_armhf.deb" || _status 1 "Unable to download vnStat"
